...
static void debug_emu_stack_limit_set(u8 mode, u32 limit_l, u32 limit_h)
{
    if (mode) {
        _EMU_SSP_H = limit_h;
        _EMU_SSP_L = limit_l;
        /*log_info("SSP_H : 0x%x", _EMU_SSP_H);
        log_info("SSP_L : 0x%x", _EMU_SSP_L);*/
    } else {
        _EMU_USP_H = limit_h;
        _EMU_USP_L = limit_l;
        /* log_info("USP_H : 0x%x", _EMU_USP_H); */
        /* log_info("USP_L : 0x%x", _EMU_USP_L); */
    }
}
...
void debug_exception_analyze(unsigned int *sp)
{
    ...
    log_info("PC: 0x%x --> 0x%x --> 0x%x --> 0x%x",
             q32DSP(0)->ETM_PC3, q32DSP(0)->ETM_PC2,
             q32DSP(0)->ETM_PC1, q32DSP(0)->ETM_PC0);

    log_info("usp   : 0x%08x", usp);
    log_info("ssp   : 0x%08x", ssp);
    log_info("sp    : 0x%08x", (u32)sp);
    log_info("Stack : 0x%08x", (u32)&_stack_end);
    log_info("rets  : 0x%08x", rets);
    log_info("reti  : 0x%08x", reti);
    log_info("retx  : 0x%08x", retx);
    log_info("rete  : 0x%08x", rete);
    log_info("psr   : 0x%08x", psr);
    log_info("icfg  : 0x%08x", icfg);

    log_info("EMU_CON = 0x%x", _EMU_CON) ;
    log_info("EMU_MSG = 0x%x", _EMU_MSG) ;
    ...
    if (_EMU_MSG & BIT(31)) {
        for (i = 0; i < 32; i++) {
            if (JL_CEMU->MSG0 & BIT(i)) {
                ...
                if ((i == 4)  || (i == 3)  || (i == 2) || (i == 1)) {
                    log_info("	at dev %s", get_dev_name(JL_CEMU->ID));
                }
            }
        }
        ...
        for (i = 0; i < 32; i++) {
            if (JL_CEMU->MSG2 & BIT(i)) {
                ...
                if ((i == 6) || (i == 2) || (i == 1)) {
                    ...
                    log_info("	mpu limit : %x", (JL_CEMU->ID & 0xf00) >> 8);
                    log_info("	at dev  : ID = %x, %s", JL_CEMU->ID & 0xff,
        ...
        if (JL_CEMU->MSG2 & BIT(0)) {
            for (i = 0; i < 32; i++) {
                if (JL_HEMU->MSG0 & BIT(i)) {
                    ...
                    if ((i == 1) || (i == 2) || (i == 3) ||
                        (i == 4) || (i == 6) || (i == 7)) {
                        log_info("	at dev %s", get_dev_name(JL_HEMU->ID));
                    }
                }
            }
            ...
            if (JL_HEMU->MSG0 & BIT(0)) {
                for (i = 0; i < 32; i++) {
                    if (JL_LEMU->MSG0 & BIT(i)) {
                        ...
                        log_info("	at dev %s", get_dev_name(JL_LEMU->ID));
                    }
                }
            }
        }
    ...
    if (_EMU_MSG & BIT(30)) {
        for (i = 0 ; i < 32 ; i++) {
            if (_ICU_EMU_MSG & BIT(i)) {
                log_info("\n[X-ICACHE] emu err msg : %s", icache_emu_msg[31 - i]);
                if (i == 3) {
                    log_info("	at dev %s", get_dev_name(_ICU_EMU_ID)) ;
                }
            }
        }
    }
    ...
    if (_EMU_MSG & BIT(29)) {
        for (i = 0 ; i < 32 ; i++) {
            if (_DCU_EMU_MSG & BIT(i)) {
                ...
                if (i == 3) {
                    log_info("	at dev %s", get_dev_name(_DCU_EMU_ID)) ;
                }
            }
        }
    }
    ...
    putchar('\n');
    for (i = 0; i < 16; ++i) {
        log_info("R%d = 0x%08x", i, sp[i]);
    }

    for (i = 0; i < ARRAY_SIZE(data_magic); i++) {
        if (data_magic[i] != 0xffffffff) {
            log_info("data_magic: %d, 0x%08x", i, data_magic[i]);
        }
    }
...
void debug_mpu_set(int idx, u32 begin, u32 end, u32 inv, const char *format, ...)
{
    if (idx > (MPU_MAX_SIZE - 1)) {
        log_info("MPU idx overflow %d", idx);
        return;
    }
...
{
    if (num > 3) {
        log_info("MPU idx overflow %d", num);
        return;
    }
...
void debug_init()
{
    ...
    /*log_info("pc_limit0: 0x%x ~ 0x%x",  _DSP_PC_LIML0 & 0x1ffffff, _DSP_PC_LIMH0 & 0x1ffffff);
    log_info("pc_limit1: 0x%x ~ 0x%x",  _DSP_PC_LIML1 & 0x1ffffff, _DSP_PC_LIMH1 & 0x1ffffff);
    log_info("pc_limit2: 0x%x ~ 0x%x",  _DSP_PC_LIML2 & 0x1ffffff, _DSP_PC_LIMH2 & 0x1ffffff);*/
