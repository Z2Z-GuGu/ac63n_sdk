...
u32 uart_close(int id, const struct uart_platform_data *data)
{
    ...
    if (id < 2) { //uart2 用于打印
        if (reg->CON0 & BIT(0)) {
            bit_clr_ie(uart_hdl[id].irq_num);
            reg->CON0 = BIT(13) | BIT(12) | BIT(10);
            gpio_disable_fun_output_port(data->tx_pin);
            gpio_set_direction(data->tx_pin, 1);
            gpio_set_die(data->tx_pin, 0);
            gpio_disable_fun_input_port(uart_hdl[id].input_func_idx);
            gpio_set_direction(data->rx_pin, 1);
            gpio_set_pull_up(data->rx_pin, 0);
            gpio_set_pull_down(data->rx_pin, 0);
            gpio_set_die(data->rx_pin, 0);
            log_info("uart%d close! CON0:%x", id, reg->CON0);
        }
        return 0;
    } else {
        return 1;
...
void uart_update_set_baud(int id, u32 baudrate)
{
    JL_UART_TypeDef *reg = uart_hdl[id].reg;
    reg->OTCNT = clk_get("lsb") / 1000;
    reg->BAUD = (uart_clk / baudrate) / 4 - 1;
}
...
static const struct uart_platform_data uart_config = {
    .id         = 0xff,//0:uart0  1:uart1  >1:自动分配空闲uart0/uart1
    .tx_pin     = IO_PORTA_02,
    .rx_pin     = IO_PORTA_03,
    .baudrate   = 115200,
};
...
void uart_loop_test()
{
    ...
    for (int j = 0; j < 100; j++) {
        ...
        if (!memcmp(uart_test_rxbuf, uart_test_txbuf, sizeof(uart_test_txbuf))) {
            log_info("async loop test suss: %d", j);
            memset(uart_test_rxbuf, 0, sizeof(uart_test_rxbuf));
        } else {
            log_info("async loop test faild:");
            put_buf(uart_test_rxbuf, sizeof(uart_test_rxbuf));
            return;
        }
    }
