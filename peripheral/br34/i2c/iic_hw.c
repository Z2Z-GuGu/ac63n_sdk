...
int hw_iic_init(hw_iic_dev iic)
{
    ...
    if ((ret = iic_port_init(iic))) {
        log_info("invalid hardware iic port");
        return ret;
    }
    hw_iic_set_die(iic, 1);
    if (iic_info_role(iic) == IIC_MASTER) {
        ...
        if ((ret = hw_iic_set_baud(iic, iic_info_baud(iic)))) {
            log_info("iic baudrate is invalid");
            return ret ;
        }
    } else {
    ...
    log_info("IIC_CON0 0x%04x", iic_regs[id]->CON0);
    log_info("IIC_CON1 0x%04x", iic_regs[id]->CON1);
    log_info("IIC_BAUD 0x%02x", iic_regs[id]->BAUD);
    //log_info("IIC_BUF %02x", iic_regs[id]->BUF);
    log_info("IOMC1 0x%08x", JL_IOMAP->CON1);
#endif
    return 0;
}
...
_slave_isr_send:
    if (iic_slave_state == IIC_STATE_SEND) {
        ...
        if (iic_slave_send_is_end(iic_regs[id])) { //主机回了nack
            ...
            /* log_info(" 	slave send: 0x%x", send_byte); */
    ...
    if (iic_slave_dma_is_enable(iic_regs[id])) {
        ...
        iic_auto_ack(iic_regs[id]);
        log_info("dma end. rx len:%d", buf_len);
...
//结构体成员: role=IIC_SLAVE
    ...
    log_info("%s icfg = 0x%x", __func__, tmp);
#if 0
    u32 con2 = 0;
    u32 *addr = NULL;
    ...
    while (1) {
        if (con2 != iic_slave_dma_buf_depth(iic_regs[0])) {
            asm("trigger");
            con2 = iic_slave_dma_buf_depth(iic_regs[0]);
            log_info("dma con2: 0x%x, data = 0x%x @ 0x%x", con2, *(addr + (con2 >> 16)), addr + (con2 >> 16));
        }
        delay(1000);
    };
#endif
...
static void hw_iic_host_int_test(void)
{
    ...
    for (int i = 0; i < 50; i++) {
        ...
        iic_kick_start(iic_regs[id]); //kick start
        log_info("%s run", __func__);
        while (flag);
    }
    log_info("%s out", __func__);
    while (1);
}
