...
/**
 * @brief plcnt引脚输出1，即给寄生电容充电，驱动内部调用函数
 * @param key ：带有当前按键引脚的结构体参数
 */
...
/**
 * @brief plcnt引脚设输入，即给寄生电容放电，驱动内部调用函数
 * @param key ：带有当前按键引脚的结构体参数
 */
...
/**
 * @brief 设置引脚与plcntmo模块的映射关系，驱动内部调用函数
 * @param key ：带有当前按键引脚的结构体参数
 */
...
/**
 * @brief plcnt当前通道的数据处理函数，包括简单滤波，阈值判断，标定常态值
 * @param cur_val : 当前通道的放电计数值
 * @param ch : 当前通道号
 */
...
void scan_capkey_async(void)
{
    ...
    old_val = JL_PCNT->VAL;
    plcnt_iomc((struct touch_key_port *)(&(user_data->port_list[cur_ch])));
    plcnt_io_in((struct touch_key_port *)(&(user_data->port_list[cur_ch])));
    if (net_ch != cur_ch) {
        plcnt_io_out((struct touch_key_port *)(&(user_data->port_list[net_ch])));
    }
    cur_ch = net_ch;
}
REGISTER_TIMER_TARGET(plcnt_timeout_det) = {
    .timer_handle = scan_capkey_async,
};
...
void scan_capkey(void)
{
    ...
    for (u8 ch = 0; ch < user_data->num; ch ++) {
        plcnt_io_out((struct touch_key_port *)(&(user_data->port_list[ch])));
        plcnt_iomc((struct touch_key_port *)(&(user_data->port_list[ch])));
...
/*************************** 以下是SDK参考示例 ****************************/
// *INDENT-OFF*
const struct touch_key_port test_touch_key_list[] = {
    //按键0
    {
        .press_delta = 100,   //变化量的阈值
        .port = IO_PORTA_01,	//任意IO
        .key_value = 0x12,      //预留的按键值
    },
    //按键1
    {
        .press_delta = 100,   //变化量的阈值
        .port = IO_PORTB_03,	//任意IO
        .key_value = 0x34,      //预留的按键值
    },
};

TOUCH_KEY_PLATFORM_DATA_BEGIN(test_touch_key_pdata)
    .num = ARRAY_SIZE(test_touch_key_list),
    .port_list = test_touch_key_list,
TOUCH_KEY_PLATFORM_DATA_END()


void plcnt_touch_key_test(void)
{
    log_info("*****************  plcnt touch key test  ***************\n");
    plcnt_init((void *)&test_touch_key_pdata);

    u8 plcnt_key_val = 0;
    extern void wdt_clr();
    while (1) {
        wdt_clr();
        plcnt_key_val = get_plcnt_key_value();
        if (plcnt_key_val != 0xff) {
            log_info("plcnt_key_val = %d\n", plcnt_key_val);
        }
        delay(100000);
    }
}
