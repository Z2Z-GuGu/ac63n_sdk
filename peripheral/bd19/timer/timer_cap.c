...
void timer_cap_isr(void)
{
    ...
    u32 cur_cnt = 0;
    for (u8 tmr_num = 0; tmr_num < 4; tmr_num ++) {
...
void timer_cap_init(JL_TIMER_TypeDef *JL_TIMERx, u32 cap_io, u8 edge, CLK_DIV_4bit clk_div, void (*cbfun)(u8, u32))
{
    ...
    request_irq(TIMERx_IRQ_IDX[tmr_num], 3, timer_cap_isr, 0);
...
void timer_cap_test(void)
{
    ...
    timer_cap_init(JL_TIMER3, IO_PORTA_01, 2, CLK_DIV_8, user_tmr3_cap_cbfun);
