...
#define USB1_DP_OUT(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(0)) | ((x & 0x1)<<0))
#define USB1_DM_OUT(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(1)) | ((x & 0x1)<<1))
#define USB1_DP_DIR(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(2)) | ((x & 0x1)<<2))
#define USB1_DM_DIR(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(3)) | ((x & 0x1)<<3))
#define USB1_DP_PU(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(4)) | ((x & 0x1)<<4))
#define USB1_DM_PU(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(5)) | ((x & 0x1)<<5))
#define USB1_DP_PD(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(6)) | ((x & 0x1)<<6))
#define USB1_DM_PD(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(7)) | ((x & 0x1)<<7))
#define USB1_DP_DIE(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(8)) | ((x & 0x1)<<8))
#define USB1_DM_DIE(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(9)) | ((x & 0x1)<<9))
#define USB1_DP_DIEH(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(10)) | ((x & 0x1)<<10))
#define USB1_DM_DIEH(x)    JL_USB1_IO->CON0 = ((JL_USB1_IO->CON0 & ~BIT(11)) | ((x & 0x1)<<11))

#define USB1_DP_IN()       (JL_USB1->CON0&BIT(17))
#define USB1_DM_IN()       (JL_USB1->CON0&BIT(16))
...
#define USB_SET(gpio, reg_name, value)	\
	do { \
		...
		if (IO_PORT_DP1 == (gpio)) { \
			USB1_DP_##reg_name(value); \
		} \
		if (IO_PORT_DM1 == (gpio)) { \
			USB1_DM_##reg_name(value); \
		} \
	} while (0)
...
{
    if (enable) {
        JL_USB1->CON0 &= ~BIT(0);//USB_PHY_OFF
...
int gpio_set_direction(u32 gpio, u32 value)
{
    u32 mask;
    struct gpio_reg *g;
    if ((gpio >= IO_PORT_DP) && (gpio <= IO_PORT_DM1)) {
        if (gpio < IO_PORT_DP1) {
            usb_iomode(1);
        } else {
            usb1_iomode(1);
...
int gpio_read_input_value(u32 gpio)
{
    ...
    if (gpio == IO_PORT_DP) {
        ...
        return USB_DM_IN();
    } else if (gpio == IO_PORT_DP1) {
        return USB1_DP_IN();
    } else if (gpio == IO_PORT_DM1) {
        return USB1_DM_IN();
    }
    g = gpio2reg(gpio);
    if (!g) {
...
u32 get_gpio(const char *p)
{
    ...
    if (strcmp(p, "USB1DP") == 0) {
        return  IO_PORT_DP1;
    }
    if (strcmp(p, "USB1DM") == 0) {
        return  IO_PORT_DM1;
    }
    return -EINVAL;
}
...
u32 *gpio2crossbar_outreg(u32 gpio)
{
    if (gpio >= IO_MAX_NUM) {
        if (gpio == IO_PORT_DP) {
            ...
            return (u32 *) & (JL_OMAP->USB1DP_OUT);
        } else if (gpio == IO_PORT_DM1) {
            return (u32 *) & (JL_OMAP->USB1DM_OUT);
        } else {
            return NULL;
        }
    }
