...
PWM_LED_PLATFORM_DATA_BEGIN(test_pwm_led_pdata)
...
#if PWM_LED_BRI_MODE
    .mode = 1,                          //占空比呼吸模式
    .mode_1.h_pwm_duty_max = 128,       //h_pwm最大占空比，即每过tc/pwm_prd=20/10=2个pwm周期，占空比变化1，占空比递增时间 = 128 * tc = 2560ms，占空比递减时间 = 128 * tc = 2560ms
    .mode_1.l_pwm_duty_max = 240,       //l_pwm最大占空比，即每过tc/pwm_prd=20/10=2个pwm周期，占空比变化1，占空比递增时间 = 240 * tc = 4800ms，占空比递减时间 = 240 * tc = 4800ms
    ...
    .mode_1.pwm_duty_0_cnt = 100,       //pwm呼吸结束后，维持高组态的时间 = 100 * tc = 100 * 20ms = 2000ms
                                        //所以h_pwm的总周期 = 2 * 2560 + 500 + 2000 = 7620ms,  l_pwm的总周期 = 2 * 4800 + 1000 + 2000 = 12600ms
#else
    .mode = 0,                          //占空比固定模式
