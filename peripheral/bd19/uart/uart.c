...
void uart_loop_test()
{
    ...
    for (int j = 0; j < 100; j++) {
        ...
        if (!memcmp(uart_test_rxbuf, uart_test_txbuf, sizeof(uart_test_txbuf))) {
            log_info("sync loop test suss: %d\n", j);
            memset(uart_test_rxbuf, 0, sizeof(uart_test_rxbuf));
        } else {
            log_error("sync loop test faild:\n");
    ...
    for (int j = 0; j < 100; j++) {
        ...
        if (!memcmp(uart_test_rxbuf, uart_test_txbuf, sizeof(uart_test_txbuf))) {
            log_info("async loop test suss: %d\n", j);
            memset(uart_test_rxbuf, 0, sizeof(uart_test_rxbuf));
        } else {
            log_error("async loop test faild:\n");
            put_buf(uart_test_rxbuf, sizeof(uart_test_rxbuf));
            return;
        }
    }
