...
#define MSYS_TO_P11_COMMON_CMD(cmd) \
	local_irq_disable();	 \
	M2P_COMMON_CMD = cmd; \
	P11_M2P_INT_SET = BIT(M2P_CCMD_INDEX); \
	do {								  \
		asm("csync");                     \
	}									  \
	while( !((P11_P2M_INT_PND & BIT(P2M_REPLY_CCMD_INDEX)) && (P2M_REPLY_COMMON_CMD == cmd)) ); \
	P11_P2M_INT_CLR = BIT(P2M_REPLY_CCMD_INDEX); \
	local_irq_enable();
