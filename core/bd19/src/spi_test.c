...
static void spi2_isr()
{
    ...
    if (spi_get_pending(spi2_hdl)) {
        ...
        if (slave_dir == 1) {
            ...
            slave_dir = 1;
            spi_recv_byte_for_isr(spi2_hdl);
        }
#else
        if (slave_dir == 1) {
...
//spi cs init
    spi_cs_init(IO_PORTB_03, 0); //spi1 cs out
...
__out_dma://delay(100);
    ...
    puts("\nspi1 master receivce buffer:");
    put_buf(spi1_recv_buf, 100);
    puts("spi2 slave receivce buffer:");
    put_buf(spi2_recv_buf, 100);
