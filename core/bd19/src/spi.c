...
#define spi_data_width(reg, x)              ((reg)->CON = (reg->CON&~(3<<10))|((x)<<10))//SPIO支持1bit , 2bit和4bit模式, SPi1/SPi2只支持1bit/2bit模式
...
static JL_SPI_TypeDef *const spi_regs[SPI_MAX_HW_NUM] = {
    JL_SPI0,  //SPI0系统已使用，SPI1 SPI2供用户使用
    JL_SPI1,
    JL_SPI2,
};
...
int spi_set_baud(spi_dev spi, u32 baud)
{
    ...
    sysclk = clk_get("spi");
    log_info("spi clock source freq: %u", sysclk);
    if (sysclk < baud) {
...
#if 0
    log_info("spi%d clk     = %d", id, clock);
    log_info("spi%d mode    = %d", id, mode);
    log_info("spi%d role    = %d", id, role);
    log_info("spi%d clk_pin = %d", id, port[0]);
    log_info("spi%d do_pin  = %d", id, port[1]);
    log_info("spi%d di_pin  = %d", id, port[2]);
    log_info("spi%d CON     = %04x", id, spi_r_reg_con(spi_regs[id]));
    log_info("spi%d IOMC1   = %08x", id, JL_IOMAP->CON1);
#endif
    return 0;
}
